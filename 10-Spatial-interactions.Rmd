...
> When you've finishing exploring, remove this dataset from ArcGIS. You
...
There are, however, a number of issues with a log-normal
...
While this works effectively with a small number of
categories, in our case there are 314 local authorities in the dataset
...
notebook**](https://doc.esri.com/en/arcgis-pro/latest/arcpy/get-started/pro-notebooks.html), which let's us programmatically access all of the ArcGIS tools, as well as some additional ones from open-source libraries. Here, the one of particular interest is [`statsmodels`](https://www.statsmodels.org/stable/index.html), which contains a range of additional functionality for statistical modelling, and will enable us to run a generalised linear regression without manually encoding our dummy variables. 
...
**GEOG71551** *Understanding GIS*), but I will talk through each part of the code in
...
- [`Arcpy`](https://doc.esri.com/en/arcgis-pro/latest/arcpy/get-started/what-is-arcpy-.html), for access to ArcGIS functions.
- [`numpy`](https://numpy.org/), for mathematical analysis.
- [`pandas`](https://pandas.pydata.org/), for data analysis and manipulation.
- [`statsmodels`](https://www.statsmodels.org/stable/index.html), for statistical modelling. 
- [`os`](https://docs.python.org/3/library/os.html), for interacting with the operating system and accessing directories and files.
...
code cells will switch from `[*]` to `[#]` to denote completion,
without errors or warnings:

![](./screenshots/practical-10/successful-code.png){width="100%"}

In the above image, we have two code cells, marked by `[1]` and `[2]`. If you make any edits to a particular cell, for example adding or removing a library in `[1]`, then subsequent cells would need to be re-run using the "play" button, as their behaviour is dependent on the previous cells. 

We can access the map layers available in `m`, and here we are
interested in the Near Table `LAD_point_neartable`, which contains all
of the fields we've been using for modelling. This is converted to a
