...
Here is a spatial representation of the Gaussian and bisquare weighting schemes, using the same bandwidth $h$ for each method:

<center>![](./images/practical-8/weighting_combined.png){width="100%"}</center>
...
**Overall**, the MGWR results provide fascinating (in my view) insights into the spatially varying processes that govern life expectancy, as summarised below. However, we should recognise that geographically weighted regression is an **exploratory** method. Much like tests for [autcorrelation](#autocorrelation_summary), the results of SVC models are dependent on the choice of parameters e.g., the selection of the kernel and local weighting scheme. In turn, while these techniques provide insight into how variable importance changes over space, they are not **objective truth**. As switching parameters will change the regression coefficients, and perhaps may fundamentally change your interpretations, care is needed to:

1. Select parameters which are appropriate for the geographic process you are studying e.g., choice of $\mathbf{W}$. 
2. Evaluate the sensitivity of your results to those parameters.
3. Present the results and interpretation cautiously. 

Within that context, deprivation and income are consistently statistically significant predictors, but their relationship with life expectancy varies spatially within the study area. Evidence of spatially varying relationships could have implications for healthcare management and policy i.e., a single, uniform policy may not be equally effective in all locations. In some areas, tackling deprivation could conceivably have the greatest impact on improving health outcomes, whereas in other areas, focusing on wealth inequality might be more effective[^caveat_intervention].
...
# Calculate distance from focal location
grid$d <- sqrt(
  (grid$x - x0)^2 +
  (grid$y - y0)^2
)
...
grid$gaussian <- f(grid$d, 5)
grid$bisquare <- fb(grid$d, 5)

# Gaussian spatial weighting
s <- ggplot(grid, aes(x = x, y = y, fill = gaussian)) +
  geom_raster() +
  coord_equal() +
  scale_fill_viridis_c(name = "Weight") +
  labs(
    x = "X",
    y = "Y",
...
ggsave(
  plot = s,
  "./images/practical-8/gaussian_spatial.png",
  height = 3,
  width = 4,
  units = "in",
  dpi = 300
...
# Bisquare spatial weighting
e <- ggplot(grid, aes(x = x, y = y, fill = bisquare)) +
  geom_raster() +
  coord_equal() +
  scale_fill_viridis_c(name = "Weight") +
  labs(
    x = "X",
    y = "Y" +
      title = "Bisquare weighting") +
  theme_classic()

# Save
ggsave(
  plot = e,
  "./images/practical-8/bisquare_spatial.png",
  height = 3,
  width = 4,
  units = "in",
  dpi = 300
...
combi_plot <- s + e + guide_area() +
  plot_layout(guides = 'collect', widths = c(1,1,0.4), axis_titles = 'collect_y')

ggsave(
  plot = combi_plot,
  "./images/practical-8/weighting_combined.png",
  height = 3,
  width = 6,
  units = "in",
  dpi = 300
