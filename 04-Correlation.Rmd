...
<center>![](./images/practical-4/non-linear-edit.png){width="80%"}</center>

<br>

::: question
What $r$ values do you think would be calculated for $A,B,C$ and $D$?
:::
...
For $A$, $r=0.05$, indicative of a slight negative correlation between $x$ and $y$.

For $B,C$ and $D$, $r=0.84$. Without inspecting the plots, we would conclude that this is indicative of strong positive correlations between $x$ and $y$. However, visual inspection of the data indicates that the actual relationship is quite different for $B,C$ and $D$. The relationship for $B$ is perfectly linear, albeit with a single outlier, whereas the relationship is much less strong for $D$. By comparison, the relationship between $x$ and $y$ for $B$ (as for $A$) is clearly non-linear, and the relationship would be better explained by a non-linear function.

The key point to **remember** is that measures of association should be interpreted critically, and in light of visual inspection of the data. The seminal work by @anscombe1973 is worth a read here[^data_viz].

[^data_viz]: ["Anscombe's quartet"](https://en.wikipedia.org/wiki/Anscombe%27s_quartet), and the more recent ["Datasaurus dozen"](https://en.wikipedia.org/wiki/Datasaurus_dozen), are great illustrations of the importance of data visualisation. 
...
Let's use an example to illustrate, using ecological data from @getzin2012, which consists of paired measurements of the median gap shape complexity index (GSCI) $x$ and species richness (SR) $y$ for a forest patch (Hainich) in central Germany[^04-correlation-5].

[^04-correlation-5]: You'll also use this dataset in **GEOG62411** *Data Acquisition for GI Scientists*. Values generated using [PlotDigitizer](https://plotdigitizer.com/), and rounded here for ease of calculation.

$$\begin{array}{cc}
\text{Median GSCI} & \text{SR}\\
\hline
1.42 & 15.86\\
1.53 & 20.79\\
1.34 & 22.82\\
...
[^04-correlation-6]: I've rounded the numerator and denominator and the resulting $r$ value to simplify presentation, so if you do decide to do the calculations yourself, the values may differ very slightly.

```{python, eval=FALSE}
# Data from Getzin et al. (2012), tab-delimited
GSCI	1.42	1.53	1.34	1.69	1.45	1.55	1.63	1.87	1.87	1.94	1.95	2.17	2.18	2.29	2.52	2.9	2.7	2.95	3.08	3.04
SR	15.86	20.79	22.82	26.82	33.86	33.95	33.86	29.79	36.84	37.86	33.86	38.79	33.86	31.83	38.87	40.83	44.82	52.89	50.93	48.05
```
...
This is also the first time we are going to **spatialise** a standard statistical approach. This is important, because standard statistical approaches, such as Pearson's $r$, typically overinflate associations and relationships when **applied to spatial data**. This can occur because these approaches only consider the numeric similarity of the paired observations, and with no consideration of geography. This is illustrated well by the examples of @lee2001, where there is a relatively strong degree of correlation for each dataset pair (i.e., $r=0.422$), even when co-pattering seems to be absent, or at least very weak (e.g., $A-C$). 

Based on the above reasoning, we are going to introduce a measure of **bivariate spatial association** developed by @lee2001, which "*captures the relationship between two variables, taking the topological relationship among observations into account*". 
...
> Load `2024_US_County_Level_Presidential_Results.csv` but this time use a **static** table join via [Join Field](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/data-management/join-field.html?tabs=dialog) to link the election results to our county geometries, joining using the `NAMELSAD` and `county_name` fields, and selecting the `gop_perc` and `dem_perc` "Transfer Fields".
...
> Use [Join Field](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/data-management/join-field.html?tabs=dialog) to produce static table joins between `us_county_500k_contig_cf` and the education, poverty and demographic tables, joining based on `GEOID_int` and `FIPS_code`.
...
> Modify the layer symbology and inspect the Attribute Table to investigate the education, poverty and demographic variables.
...
> Create your own visualisations for the education, poverty and demographic variables. For example, "Bivariate Colors" in Primary Symbology settings allows for the symbolising of **two** factors simultaneously.
...
> Using tools of your choice, investigate **global** and **local** spatial autcorrelation for the education, poverty and demographic variables. I would recommend focusing on the median household income, the percentage of the total population classed as "white", and the percentage of adults with a bachelor's degree or higher.
...
$$L_{XY}=\frac{n}{\sum_i(\sum_{j}w_{ij})^2}\frac{\sum_{i}[(\sum_{j}w_{ij}(x_{j}-\bar{x}))(\sum_{j}w_{ij}(y_{j}-\bar{y}))]}{\sqrt{\sum_{i}(x_{i}-\bar{x})^2}\sqrt{\sum_{i}(y_{i}-\bar{y})^2}}$$ This **looks** quite complicated (and it took me a while to untangle the mathematics!), but there are lots of components here that you should be familiar with[^04-correlation-10].

[^04-correlation-10]: If you read @lee2001, you'll notice the notation in Equation 12 is slightly different to that provided above. I've made some small changes (not affecting the calculations) for consistency with other equations we've used previously. For example, $w_{ij}$ relates to the values in the spatial weights matrix, introduced [here](#spatial_lag), which is defined by @lee2001 as $v_{ij}$.
...
Luckily we don't need to do any further calculations, because the global value for $L$ is calculated based on feature-specific "local" $L$ values. We've already covered how those are calculated in some detail, including in the [schematic](#local_l_schematic) above. These local $L$ values indicate whether the focal feature is characterised by positive, negative or negligible local spatial association.
...
> Open the Geoprocessing tool [Bivariate Spatial Association (Lee's L)](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/spatial-statistics/bivariate-spatial-association.html?tabs=dialog), using `us_county_500k_contig_cf` as the "Input Features", with `per_gop` and `PCTPOVALL_2023` as the analysis fields[^04-correlation-13] and use Queen's contiguity. Save the output features to the project directory with a suitable name e.g., `L_gop_pov_qc`

[^04-correlation-13]: Unlike regression, the order of the variables is not important here i.e., correlation $\text{correlation}\:A-B=\text{correlation}\:B-A$.

The tool has produced a striking map of local $L$, but before we explore it, we need to investigate the global results, which are shown below. These can be found in the tool output &rarr; "Messages":

<center>![](./screenshots/practical-4/gop_poverty_L.png){width="80%"}</center>
...
While we should be cautious about the local statistics in light of this, the Attribute Table still contains some useful information, such as local $L$ and the neighbourhood averages for each variable[^04-correlation-14].

[^04-correlation-14]: Note these are not exactly the same as the **normalised** spatial lags we calculated above i.e., $\sum_{j}w_{ij}(y_{j}-\bar{y})$. They appear to be $\sum_{j}w_{ij}y_{j}$.

::: question
What is the local $L$ for our test county (Sante Fe County, GEOID: 35049)? Why does this differ from the [calculations](#local_l_schematic) above?
:::
...
![*Local spatial association for Republican vote share vs. Share of the population with a Bachelors degree or higher*](./screenshots/practical-4/lisa-gop-bachelors.png){width="100%"}
...
<center>![](./screenshots/practical-4/lee-scatter.png){width="60%"}</center>
...
> Repeat the analysis, looking at the percentage of the population classed as "white" (`WA_perc`) and the median household income (`MEDHHINC_2023`), and any other bivariate comparisons of interest.
...
> Run [Bivariate Spatial Association (Lee's L)](https://doc.esri.com/en/arcgis-pro/latest/tool-reference/spatial-statistics/bivariate-spatial-association.html?tabs=dialog) using a distance based or $k$ neighbours weighting to evaluate this.
...
#------------ Code to generate plot of Getzin et al. (2012) data
